...
export class EditMembersVolunteersComponent implements OnInit, OnDestroy {

  EditMembersForm: FormGroup; 
  submitted = false;
...
Agancies= [
  ...
  radioChangeHandelerGender(event:any){
    this.EditMembersForm.value.gender = event.target.value;
  }


  //to get the value of radio button status 
  radioChangeHandelerNews(event:any){
    this.EditMembersForm.value.news_letter = event.target.value;
  }

  //to get the value of radio button status 
   radioChangeHandelerStatus(event:any){
    this.EditMembersForm.value.status = event.target.value;
  }
  ...
  ngOnInit(): void {
    ...
    this.EditMembersForm = this.formBuilder.group({
      id:                   [this.Id],
      type:                 [''],
      agancey:              [''],
      acc_id:               [''],
  ...
  get f() { return this.EditMembersForm.controls; }
  ...
  onSubmit() {
    ...
    if (this.EditMembersForm.invalid) {
        return;
    }

    this.apiService.EditMemberRequest(this.EditMembersForm.value)
        .pipe(first())
        .subscribe(
            data => {
